<template>
    <div v-if="tooltip.isTooltipLoaded" :style="getStyle()" class="tooltip">
        <span class="tooltip-main-text">{{ tooltip.tooltip.content }}</span>
        <span class="tooltip-shortcut-text">{{ tooltip.tooltip.shortcut }}</span>
    </div>
</template>
...
const getStyle = () => {
    return {
        ...getPosition(),
        backgroundColor: tooltip.value.tooltip.style.backgroundColor.toHex(),
        color: tooltip.value.tooltip.style.color.toHex()
    }
}

const getPosition = () => {
    const anchor = tooltip.value.tooltip.anchor;
    const bb = tooltip.value.anchorElement;
